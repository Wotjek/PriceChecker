...
3. **Blind spoty** (`fill_blind_spots`) — domeny, które odrzuciły pobranie (i requests, i przeglądarkę; np. bike24 = Akamai, r2-bike = Cloudflare), dostają cenę z Google: najpierw feed Google Shopping przez SerpAPI (`settings.shopping_budget` zapytań/run, domyślnie 4; `settings.serpapi_reserve`, domyślnie 10, chroni resztę limitu), potem indeks Googlebota przez Programmable Search (`GOOGLE_API_KEY`+`GOOGLE_CX`, `settings.cse_budget`/run, domyślnie 15; 100 zapytań/dzień darmowe, cena może być sprzed kilku dni). Filtry jakości na tytule oferty (`_title_ok`: tokeny produktu + `require_tokens` + wariant). Takie oferty mają `via=shopping|google` w `all_offers.csv` i znacznik „Google" w audycie dashboardu.
