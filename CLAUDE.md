...
Plan Free: 250 zapytań/mies. Pełny FIRE przy ~10 produktach ≈ 90 zapytań (frazy × kraje × silniki + Shopping). Timeout SerpAPI i tak zużywa zapytanie (stąd `SERPAPI_TIMEOUT=60`). Do testów zmian w monitoringu/dashboardzie używaj Light FIRE — nie zużywa limitu. Stan limitu: `data/serpapi_quota.json`.
